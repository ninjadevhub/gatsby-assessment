...
export default function PressRelease({
  ...
  if (frontmatter.banner) {
    thumbnail = url.substring(0, url.length - 1) + images.edges.filter(
      (image) =>
        image.node.name ===
        frontmatter.banner.split('/')[1].replace(/\.[^/.]+$/, '')
    )[0].node.childrenImageSharp[0].fixed.src;
  } else {
    thumbnail = url.substring(0, url.length - 1) + images.edges.filter(
    ...
    <>
      <div className="flex flex-col min-h-screen overflow-hidden">
        <Helmet>
          ...
          <meta property="twitter:description" content={frontmatter.blurb} />
        </Helmet>
...
export const pageQuery = graphql`
  query ($slug: String!) {
    ...
    blogImages: allFile(filter: {relativeDirectory: {eq: "press-release-assets"}}) {
    edges {
      node {
        id
        name
        childrenImageSharp {
          fixed(width: 600, fit: CONTAIN) {
            src
          }
        }
      }
