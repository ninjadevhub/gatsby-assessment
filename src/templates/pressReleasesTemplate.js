...
export default function PressReleasesList({ data }) {
  ...
  const mainCover = getImage(
    images.edges.filter(
      (image) =>
        image.node.name ===
        posts[0].node.frontmatter.banner.split('/')[1].replace(/\.[^/.]+$/, '')
    )[0].node
  );
                ...
                <div className="py-8 md:py-16">
                  <article className="max-w-sm mx-auto space-y-5 md:max-w-none md:flex md:items-center md:space-y-0 md:space-x-8 lg:space-x-16">
                    ...
                    <Link
                      ...
                      to={`/press-release/${mainPost.node.frontmatter.slug}`}
                      data-aos="fade-down"
                    >
